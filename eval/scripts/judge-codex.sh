...
usage() {
  cat >&2 <<'USAGE'
Usage: judge-codex.sh --task-id <id> [--track <track>] (--bundle-file <path> | --bundle <text>) (--rubric-file <path> | --rubric <text>)

The Codex judge must return JSON with scores.A, scores.B, scores.C, and rationale.
This helper validates and normalizes that output to:
{ "judge": "codex", "taskId": "...", "track": "...", "scores": {"A": n, "B": n, "C": n}, "rationale": "..." }.
USAGE
}

die_usage() {
  printf 'judge-codex: %s\n' "$*" >&2
  usage
  exit 1
}
...
require_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    printf 'judge-codex: required command not found on PATH: %s\n' "$1" >&2
    exit 4
  fi
}
...
# codex sometimes wraps its JSON in prose/markdown; extract the score object
...
if ! run_and_extract; then
  if ! run_and_extract; then
    printf 'judge-codex: model output was not valid judge JSON (after retry)\n' >&2
    exit 3
  fi
fi
