...
WRAPPER="$SCRIPT_DIR/../../scripts/run-codex-impl.sh"
ORIGINAL_PATH="$PATH"
...
Summary: Impl wrapper parsed the output-last-message file.
Verdict: PASS
Findings:
FilesTouched:
- orchestration/tests/scripts/run-codex-impl.test.sh
=== END-CONTRACT ===
EOF
}
...
case "${CODEX_MOCK_MODE:-pass}" in
  ...
  stdout_fallback)
    ...
    cat <<'EOF'
=== ORCHESTRATION-CONTRACT ===
Summary: Impl wrapper used isolated stdout as the empty-last-message fallback.
Verdict: PASS
Findings:
FilesTouched:
- orchestration/tests/scripts/run-codex-impl.test.sh
...
test_happy_path_and_prompt() {
  local name="happy path parses last message and embeds full contract"
  ...
  if ! jq -e '.summary == "Impl wrapper parsed the output-last-message file." and .verdict == "PASS"' "$stdout_file" >/dev/null; then
  ...
  assert_argv_has_pair "$argv_file" "-s" "workspace-write" || {
  ...
  for expected in \
    ...
    "FilesTouched:" \
    "=== END-CONTRACT ==="; do
    assert_file_contains "$prompt_file" "$expected" || {
      fail "$name" "prompt missing literal contract line: $expected"
      return
    }
  done
...
test_logs_real_plan_vs_synthetic() {
  ...
  local real_log="$root_real/.temp/plan-mode/active/$plan_id/logs/codex-impl-$step_id.log"
  local synth_log="$root_synth/.temp/plan-mode/logs/$plan_id/codex-impl-$step_id.log"
  local status
