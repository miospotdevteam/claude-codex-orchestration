...
description: "Build immersive, motion-first web experiences with WebGL, Three.js, R3F, GSAP ScrollTrigger, GLSL shaders, and scroll-driven 3D choreography. Use for cinematic or theatrical sites, creative-development work, full-canvas experiences, particle systems, shader and distortion effects, smooth-scroll scenes, preloaders, parallax depth, magnetic cursors, text reveals, marquees, camera flythroughs, glow, chromatic aberration, bloom, film grain, and Awwwards/Codrops-style experiences. Also trigger when the user references studios such as Active Theory, Lusion, or Immersive Garden. Do not use for standard UI components, form-heavy pages, admin dashboards, simple CSS hover states, or basic Framer Motion transitions; use frontend-design instead."
---
