...
### Step 3.2: Manual Verification with a Target MCP Client

Add the server to the target client's MCP configuration and test with real
prompts. When Claude Desktop is the product target, its configuration shape is:
...
1. Run it against the MCP server through an allowed target client or model host
