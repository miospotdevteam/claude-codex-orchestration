...
description: "Create distinctive, production-grade web interfaces with intentional typography, color, hierarchy, and visual direction. Use for building or redesigning websites, pages, landing experiences, portfolios, dashboards, marketing and product surfaces, forms, documentation, or UI components—especially when the user asks for a modern, polished, premium, beautiful, professional, clean, fresh, or less-generic result. Covers greenfield work and extensions to existing design systems in HTML/CSS, React, Vue, Svelte, Tailwind, and shadcn/ui. Do not use for a CSS bug with no design change, backend-only work, or WebGL/Three.js/shader/scroll-driven immersive experiences (use immersive-frontend). If brainstorming already established the visual direction, begin at implementation."
---
...
1. **Typography**: Display font + body font, with enough weights and language
   coverage for the product
2. **Color**: Start from a curated palette, not from scratch.
   Pick a palette that matches your Temperature axis and brand context:
   ...
   (`palx`) when the project already permits those dependencies.
...
or asking the user to choose between options, use the host's structured input
control when available; otherwise present a compact conversational choice.
Do NOT proceed to Phase 3 until the user approves the direction. If the user
disagrees, iterate on the specific choices they reject.
...
Run the project's own commands first (check `package.json`, `Makefile`, and
the repository's active instruction files). Supplement with:
