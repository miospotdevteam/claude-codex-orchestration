---
name: svg-art
description: "Generate distinctive, production-quality SVG artwork inline in code with no external image assets or libraries. Use for decorative backgrounds, abstract or hero illustrations, generative patterns, textures, grain and noise, blobs, topographic lines, mesh gradients, icons, brand marks, section dividers, filters, duotone and glow effects, data visualizations, sparklines, inline charts, and animated SVG elements. Also use when frontend-design calls for custom illustration or textured artwork. Do not use for GSAP-driven SVG animation (use immersive-frontend), raster image editing, CSS-only effects that do not need SVG, or simple geometric shapes with no artistic direction."
---
